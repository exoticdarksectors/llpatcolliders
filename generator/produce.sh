...
# Usage: bash produce.sh <cmnd_file> <target_events> <output_name> [n_jobs] [events_per_job] [--outdir DIR]
#
# Event-targeted parallel production driver.
# Stops when accumulated generated events reaches target_events.
# Merges LLP and daughter CSVs with event-id renumbering.
#
# Examples:
#   bash generator/produce.sh alps/cmnd/heavy_alp.cmnd 50000 alp_heavy_m15 4
#   bash generator/produce.sh dark_photon/cmnd/heavy_dp_m2.cmnd 10000 dp_heavy_m2 4 --outdir output/dark_photon

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

CMND="${1:?Usage: $0 <cmnd_file> <target_events> <output_name> [n_jobs] [events_per_job] [--outdir DIR]}"
TARGET_EVENTS="${2:?}"
NAME="$(basename "${3:?}")"

# Parse optional --outdir from any position in remaining args
CUSTOM_OUTDIR=""
POSITIONAL=()
shift 3
while [[ $# -gt 0 ]]; do
...
DAU="${DATADIR}/${NAME}_daughters.csv"
...
while (( total_gen < TARGET_EVENTS )); do
  ...
  for idx in "${!job_seeds[@]}"; do
    ...
    if [[ -f "$f" ]]; then
      ...
      # If the CSV is absent (generator crash) the meta is stale and must not
      # inflate n_generated.
...
# Final counts from merged CSVs
count_llp=$(tail -n +2 "$CSV" | wc -l | tr -d ' ')
count_dau=$(tail -n +2 "$DAU" | wc -l | tr -d ' ')
...
echo "  LLP rows: $count_llp | daughter rows: $count_dau | events with LLP: $total_with_llp"
