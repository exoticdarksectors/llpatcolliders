...
Uses the shared tunnel mesh and ray-casting from gargoyle_geometry.
...
For a muon at distance r:  Phi(r) = Phi_mQ x (33/r)^2

Approach:
  1. Build the tunnel fiducial mesh (shared gargoyle_geometry)
...
Usage:
  python background_trident.py [--outdir output/higgs] [--n-rays 500000]
"""
...
class TunnelBackgroundMC:
    ...
    def _setup_angular_bounds(self):
        ...
        print(f"    Bounding deta x dphi = {d_eta:.3f} x {d_phi:.4f} = {self.solid_angle_eta_phi:.4f}")
    ...
    def _sample_v(self, E_muon):
        """Rejection-sample v from d sigma / dv ~ (1/v)(1 - 4v/3 + v^2)."""
...
def plot_results(results, save_path=None):
    ...
    if trident_data:
        ...
        ax4.axvline(2*E_CUT, color='k', ls='--', label=f'2 x E_cut')
    ...
    ax6.set_xlabel('mu to Closer Electron (mm)'); ax6.set_ylabel('Events')
    ...
    if trident_data:
        ...
        ax8.set_ylabel('Weighted rate (per fb^-1)')
...
def main():
    parser = argparse.ArgumentParser(
        description='Beam muon trident background MC (full tunnel geometry)')
    parser.add_argument('--outdir', default='output',
                        help='Output directory (default: output)')
    parser.add_argument('--n-rays', type=int, default=500_000,
                        help='Number of muon directions to sample (default: 500000)')
    ...
    results = mc.run(n_rays=args.n_rays)
    ...
    print(f"  Per 3000 fb^-1 (HL-LHC, before muon veto): "
    ...
    print("\nGenerating plots...")
    save_path = os.path.join(img_dir, 'background_fullgeo_mc.png')
